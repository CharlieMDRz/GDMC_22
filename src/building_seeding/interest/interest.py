...
from parameters import SEED_COUNT, REPLACE_PARCEL_TYPE_EXPLORATION, AVERAGE_PARCEL_SIZE
from terrain import TerrainMaps, ObstacleMap
from utils import bernouilli, Point
...
class InterestSeeder:
    ...
    def try_to_reuse_existing_parcel(self, new_type) -> BuildingType:
        ...
        """
        parcels = [_ for _ in self.__parcels if _.building_type not in {BuildingType.ghost, new_type, BuildingType.station}]
    ...
    def get_optimal_type(self, seed):
        ...
        if max_interest == -1:
            ...
            return random.choice(best_types)
...
class InterestMap:
    ...
    def __init__(self, building_type: BuildingType, scenario: str, terrain_maps: TerrainMaps, districts: Districts):
        ...
        # Weights
        w = BUILDING_ENCYCLOPEDIA[scenario]
    ...
    def get_seed(self, max_iteration=None, perc=95):
        ...
        reachable_high_interest = np.percentile(self.__interest_value, perc)
    ...
    def __compute_fixed_interest(self, maps: TerrainMaps) -> np.ndarray:
        """
        ...
        # todo: could vectorize most of the double loops below
        ...
        temp_interest_map = np.vectorize(lambda d: soft_balance(d, *self.__lambdas["Temperature"]))(maps.biome.temperature_map)
        tree_interest_map = np.vectorize(lambda d: attraction_repulsion(d, *self.__lambdas["TreeDistance"]))(maps.trees.tree_distance)
        if self.__districts is None:
            density_interest_map = np.ones(size)
        else:
            density_interest_map = density(self.__size, self.__districts, self.__lambdas["Density"])

        for x, z, in itertools.product(range(size[0]), range(size[1])):
            interest_functions = np.array([
                density_interest_map[x, z],
                altitude_interest_map[x, z],
                river_interest_map[x, z],
                ocean_interest_map[x, z],
                lava_interest_map[x, z],
                steep_interest_map[x, z],
                temp_interest_map[x, z],
                tree_interest_map[x, z]
            ])
    ...
    def get_interest(self, seed, parcels=None):
        ...
        """
        if parcels is None:
            return self.__interest_value[seed.xz]
    ...
    def notify_type_change(self, parcels, parcel, old_type):
        ...
        self.__social = sociability(self.type, self.scenario, parcels[:self.__known_seeds], self.__size)
    ...
    def accessibility(self):
        return np.array(self.__access)

    @property
    def sociability(self):
        return np.array(self.__social)
    
    @property
    def terrain_interest(self) -> np.ndarray:
        return self.__fixed_interest[:]

    @property
    def map(self):
        return np.array(self.__interest_value)
